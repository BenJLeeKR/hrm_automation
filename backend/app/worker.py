"""배치 스케줄러 진입점 (로드맵 §8 "HR_AVAIL_SNAP_GEN"/"PJT_ASGN_END_ALERT"/
"HR_DATA_QUALITY_CHK" 배치 구현, 설계서 §10 자동화 배치, Phase 7 산출물 `worker.py`) —
`docker-compose.yml`의 `worker` 서비스가 `python -m app.worker`로 이 모듈을 실행한다.

현재는 배치 5종(설계서 §10.1) 중 `HR_AVAIL_SNAP_GEN`(가동가능 스냅샷 생성)·
`PJT_ASGN_END_ALERT`(투입 종료 예정 알림)·`HR_DATA_QUALITY_CHK`(데이터 품질 점검) 3종을
구현한다 — 나머지(`PJT_WEEKLY_RPT`/`SYS_DB_BACKUP`)는 §4 Phase 7 "주요 작업" 표에 남은
순서대로 후속 작업으로 진행한다.
"""
...
def _job_data_quality_chk() -> None:
    db = SessionLocal()
    try:
        history = run_data_quality_chk(db)
        logger.info("HR_DATA_QUALITY_CHK 완료: %s", history.RSLT_SUMR)
    except Exception:
        logger.exception("HR_DATA_QUALITY_CHK 실행 중 오류가 발생했습니다.")
    finally:
        db.close()
...
def main() -> None:
    ...
    scheduler.add_job(
        _job_data_quality_chk,
        CronTrigger(day_of_week="fri", hour=18, minute=0, timezone=_KST),
        id="hr_data_quality_chk",
        name="HR_DATA_QUALITY_CHK",
        misfire_grace_time=3600,
    )
    logger.info(
        "배치 스케줄러 시작 — HR_AVAIL_SNAP_GEN: 매일 01:00(KST), PJT_ASGN_END_ALERT: 매주 금 17:00(KST), "
        "HR_DATA_QUALITY_CHK: 매주 금 18:00(KST)"
    )
    scheduler.start()
