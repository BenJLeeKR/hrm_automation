...
| v0.2 | 2026-07-01 | Docker Compose 서비스 완성(db, nginx, redis 추가), 누락 테이블 설계 추가, ERD 보완, API 엔드포인트 보강, Nginx 설정 예시 추가, Next.js Dockerfile standalone 최적화, .env 보강, 보안 항목 보강, LLM/RAG 방향 보강, 프로젝트 구조 보강, 체크리스트 보완 |
...
|---|---:|---:|---|
| Nginx | `hrm-nginx` | 80 / 443 | 리버스 프록시, TLS 종단, 정적 자산 서빙 |
| Next.js | `hrm-web` | 3000 | 웹 클라이언트 |
| FastAPI | `hrm-api` | 8000 | REST API, 인증, 업무 로직 |
| PostgreSQL | `hrm-db` | 5432 | 업무 데이터 저장소(기존 설치된 PostgreSQL 활용) |
| Redis | `hrm-redis` | 6379 | 캐시, 비동기 작업 큐 |
| Worker | `hrm-worker` | - | 배치, 리포트, 알림 |

> **외부 노출 원칙:** Nginx(80/443)만 외부에 노출한다. FastAPI(8000), Next.js(3000), PostgreSQL(5432), Redis(6379)는 Docker 내부 네트워크에서만 통신하며 호스트 포트를 직접 바인딩하지 않는다.
...
| 단계 | 권장 구성 |
|---|---|
| MVP | FastAPI + PostgreSQL + Next.js + Nginx + Docker Compose |
| 운영 안정화 | HTTPS + Backup + Audit Log + Redis 캐시 |
| 확장 단계 | Celery + AI/RAG + SSO + CI/CD |
...
| `users` | 로그인 사용자 (직원과 1:1 또는 독립 관리) |
| `roles` | 권한 그룹 |
| `audit_logs` | 변경 이력 |
| `batch_jobs` | 배치 실행 이력 |
...
> **`employees ↔ users` 관계 설명:** 시스템 로그인 계정(`users`)과 직원 마스터(`employees`)는 분리 관리한다. 외부 협력사 계정, 퇴직 후 열람 목적 계정 등에 유연하게 대응하기 위해 1:1 연결은 선택(optional)으로 설계한다. `users.employee_id` 컬럼으로 연결하되 NULL 허용.
...
| `parent_id` | UUID | FK (self) | 상위 팀 (계층 구조 지원, NULL=최상위) |
...
|---|---|---|---|
| `id` | UUID | PK | 직급 ID |
| `name` | VARCHAR(100) | NOT NULL | 직급명 (예: 사원, 대리, 과장, 부장) |
...
| Method | Endpoint | 설명 |
|---|---|---|
| GET | `/health` | 헬스 체크 (Nginx, Docker health check 대상) |
...
| Method | Endpoint | 설명 |
|---|---|---|
| GET | `/api/v1/employees` | 직원 목록 조회 (페이지네이션, 필터) |
...
| `/employees` | 직원 목록 | 검색, 필터, 등록, 수정, Excel Import/Export |
...
> **포트 노출 정책:** `ports`는 Nginx 컨테이너에만 적용한다. API, Web, DB, Redis는 `expose`만 사용하여 컨테이너 간 통신만 허용하고 호스트 포트에 직접 바인딩하지 않는다.
...
http {
    ...
    server {
        listen 80;
        server_name hrm.example.internal;
    ...
    server {
        ...
        ssl_certificate     /etc/nginx/certs/hrm.crt;
        ssl_certificate_key /etc/nginx/certs/hrm.key;
...
# 헬스체크: Docker Compose healthcheck 보조용
...
### 8.5 Next.js Dockerfile 예시 (standalone 최적화)
...
# standalone 빌드 결과만 복사 (이미지 크기 대폭 절감)
...
> `next.config.js`에 `output: 'standalone'` 옵션을 반드시 추가해야 위 Dockerfile이 동작한다.
>
> ```js
> // next.config.js
> const nextConfig = {
...
# CORS (허용할 Origin 목록, 쉼표 구분)
CORS_ORIGINS=https://hrm.example.internal
...
```bash
# crontab 또는 배치 Worker에서 실행
docker compose exec -T db pg_dump -U hrm_user -d hrm \
  | gzip > /backup/hrm/hrm_$(date +%Y%m%d_%H%M%S).sql.gz

# 14일 이전 파일 자동 삭제
find /backup/hrm -name "*.sql.gz" -mtime +14 -delete
```
...
MVP에서는 RAG 없이 SQL 조회 결과를 컨텍스트로 LLM에 전달하는 방식으로 구현한다. 운영 안정화 후 다음 순서로 RAG를 도입할 수 있다.
...
6. 전달된 데이터에 없는 내용을 창작하거나 추론하지 않는다.
```
...
| Access Token 만료 | 60분 |
| Refresh Token 만료 | 7일 |
| Refresh Token 저장 위치 | DB 또는 Redis (HttpOnly Cookie로 클라이언트 전달 권장) |
...
- 운영 DB 포트(5432), Redis 포트(6379)는 외부에 직접 노출하지 않는다. Docker 내부 네트워크로만 통신한다.
- 관리자 계정은 MFA 적용을 권장한다.
- HTTPS는 운영 환경에서 필수 적용한다. HTTP 접근은 Nginx에서 HTTPS로 강제 리다이렉트한다.

**인증 및 비밀번호:**
- 비밀번호는 `bcrypt` 또는 `argon2`로 해시 저장한다. 평문 저장 금지.
- 비밀번호 정책: 최소 8자, 영문+숫자+특수문자 혼합 권장.
- 비밀번호를 운영 로그, 감사 로그에 절대 출력하지 않는다.
- JWT Secret Key는 최소 32자 이상의 랜덤 문자열로 설정한다.

**API 보안:**
- CORS 정책: `CORS_ORIGINS` 환경변수에 허용 Origin을 명시적으로 설정한다. 와일드카드(`*`) 사용 금지.
- Rate Limiting: 로그인 API는 IP 기준 분당 10회 이하로 제한하여 무차별 대입 공격을 방지한다.
- SQL Injection: SQLAlchemy ORM 및 파라미터 바인딩을 사용하고 raw SQL 직접 포맷팅 금지.

**데이터 보호:**
- 직원 개인정보 필드는 최소화한다.
- AI 질의응답에는 권한 필터링된 결과만 전달한다.
...
| 2주차 | DB/백엔드 기반 | PostgreSQL 스키마, Alembic, FastAPI 기본 API, Docker Compose 전체 구성 |
...
- [ ] Nginx 리버스 프록시 구성
- [ ] Docker 내부 네트워크 구성 (db/redis 포트 외부 미노출 확인)
...
- [ ] 핵심 테이블 생성 (employees, teams, positions, skills, employee_skills, projects, assignments, users, roles, audit_logs, batch_jobs, resource_requests, recommendation_results, availability_snapshots)
- [ ] Seed 데이터 입력 (roles, positions 기본 코드)
- [ ] DB 백업 스크립트 작성
- [ ] 복구 테스트 완료
...
| 보안 | 직원 개인정보 노출 | 높음 | RBAC, 감사 로그, 최소 정보 노출, HTTPS 강제 |
...
│   │   └── utils.ts
│   ├── package.json
│   ├── next.config.js          # output: 'standalone' 필수 설정
│   └── Dockerfile
│
├── nginx/
│   ├── nginx.conf
│   └── certs/                  # TLS 인증서 (Git 제외)
│
├── docker-compose.yml
├── .env.example
├── .gitignore
