...
| v0.7 | 2026-07-05 | 운영팀 확인 결과 반영 — ① 투입률 100% 초과 정책 차단→경고+허용으로 변경(§5.5·SCR-009), ② `SYS_CONFIG` 테이블 신규(알림 채널 DB 관리, §5.3.17), ③ `GET/PUT /api/v1/settings/notification` API 신규(§6.4), ④ "일반 설정" 화면 범위 제외 확정(조직 정보 불필요·가동률 정책 고정·단일 테넌트), ⑤ §15 리스크 업데이트, ⑥ `SYS_CONFIG` 암호화 키 관리 방식 보완(§5.3.17) |
...
erDiagram
    ...
    SYS_CONFIG }|..|| SYS_CONFIG : "standalone (no FK)"
```

> **`SYS_CONFIG`** 는 다른 테이블과 FK 관계가 없는 독립 설정 테이블이다. 알림 채널(Teams Webhook, SMTP) 설정값을 키-값 형태로 저장하며 관리자가 웹 화면에서 수정한다.
...
#### 5.3.17 `SYS_CONFIG` — 시스템 설정 *(신규)*

시스템 운영에 필요한 설정값을 키-값(Key-Value) 형태로 저장하는 독립 테이블이다. 다른 테이블과 FK 관계가 없으며 관리자가 웹 화면에서 직접 수정한다.

> **설계 배경 (2026-07-05 운영팀 확인):**
> - 알림 채널(Teams Webhook, SMTP) 설정을 `.env` 환경변수가 아닌 웹 화면에서 관리하기로 확정
> - "일반 설정" 화면의 나머지 후보 항목들(조직 정보·가동률 임계값·배치 실행 시각·30일 기준)은 변경 필요 없음으로 확인 → 하드코딩 유지, `SYS_CONFIG` 저장 불필요
> - 단일 테넌트로 확정 → 조직 정보 테이블 불필요

| 컬럼명 | 타입 | 제약 | 설명 |
|---|---|---|---|
| `CONFIG_KEY` | VARCHAR(100) | PK | 설정 키 (예: `notification.teams_webhook_url`) |
| `CONFIG_GRP` | VARCHAR(50) | NOT NULL | 설정 그룹 (`NOTIFICATION`) |
| `CONFIG_NM` | VARCHAR(200) | NOT NULL | 한글 표시명 |
| `CONFIG_VAL` | TEXT | NULL | 설정값 (미설정 시 NULL — `.env` 값으로 폴백) |
| `IS_SECRET` | BOOLEAN | DEFAULT FALSE | 비밀값 여부 (TRUE이면 AES 암호화 저장) |
| `CONFIG_DESC` | TEXT | NULL | 설명 |
| `UPD_DTTM` | TIMESTAMPTZ | NOT NULL | 수정일시 |
| `UPD_USER` | VARCHAR(100) | NULL | 수정자 |

**초기 Seed 데이터 (알림 채널 그룹):**

| `CONFIG_KEY` | `CONFIG_NM` | `IS_SECRET` | 초기값 |
|---|---|---|---|
| `notification.teams_webhook_url` | Teams Webhook URL | FALSE | NULL |
| `notification.smtp_host` | SMTP 호스트 | FALSE | NULL |
| `notification.smtp_port` | SMTP 포트 | FALSE | NULL |
| `notification.smtp_user` | SMTP 사용자 | FALSE | NULL |
| `notification.smtp_password` | SMTP 비밀번호 | **TRUE** | NULL |
| `notification.email_from` | 발신 이메일 주소 | FALSE | NULL |

> **폴백(Fallback) 정책:** `CONFIG_VAL`이 NULL이면 `.env`의 해당 환경변수(`TEAMS_WEBHOOK_URL`, `SMTP_HOST` 등)를 사용한다. DB 값이 설정되면 DB 값이 우선한다. 이 정책으로 기존 `.env` 운영환경을 유지하면서 점진적으로 웹 관리로 전환할 수 있다.
>
> **`IS_SECRET=TRUE` 처리:** 저장 시 AES-256으로 암호화, 조회 API 응답에서 값을 마스킹(`*` 처리), `SYS_AUDIT_LOG`에 실제 값 대신 `"[SECRET_UPDATED]"` 기록.
>
> **암호화 키 관리 (2026-07-05 보완):** 암호화 키는 애플리케이션 서버의 `.env` 환경변수 `CONFIG_ENCRYPTION_KEY`(32바이트, base64 인코딩)로 관리하며 DB에는 저장하지 않는다. 키 회전(rotation) 시에는 기존 `CONFIG_VAL`을 구 키로 복호화 후 신 키로 재암호화하는 별도 배치/스크립트가 필요하며, MVP 범위에서는 키 회전 절차를 구현하지 않고 수동 처리(운영팀이 키 변경 시 알림 채널 값 재입력)로 대체한다.
...
| `ALLOC_RT` 합계 검증 | 동일 기간 동일 사원의 `PJT_ASGN_HIS.ALLOC_RT` 합계가 100% 초과 시 **경고 표시 후 저장 허용** (제안중 단계 초과 등록 허용 — 2026-07-05 운영팀 확인) |
...
| Method | Endpoint | 설명 |
|---|---|---|
| GET | `/api/v1/settings/notification` | 알림 채널 설정 조회 (`SYS_CONFIG WHERE CONFIG_GRP='NOTIFICATION'`) — `IS_SECRET=TRUE` 항목은 값 마스킹 |
| PUT | `/api/v1/settings/notification` | 알림 채널 설정 저장 — `IS_SECRET=TRUE` 항목 AES-256 암호화, `SYS_AUDIT_LOG` 기록 |
| POST | `/api/v1/settings/notification/test` | 알림 채널 테스트 발송 (Teams 또는 Email 1건 전송) |

> **"일반 설정" 화면 API 없음 (2026-07-05 확정):**
> 조직 정보·가동률 임계값·배치 실행 시각·알림 기준 일수는 모두 하드코딩 고정값으로 운영팀 확인. 해당 항목에 대한 `SYS_CONFIG` 저장 및 설정 API는 설계하지 않는다.
...
| `PJT_ASGN_HIS.ALLOC_RT` | 동일 기간 동일 `EMPL_ID`의 합계 > 100% 시 경고 표시 후 저장 허용 (2026-07-05 확정) |
...
| 보안 | `SYS_CONFIG` 알림 채널 비밀값 노출 | 중간 | `IS_SECRET=TRUE` 항목 AES-256 암호화 저장, API 응답 마스킹, `SYS_AUDIT_LOG` 기록 시 `[SECRET_UPDATED]` 처리 |
...
| ~~설정~~ | ~~"일반 설정" 화면 범위 미정의~~ | ~~해소~~ | **2026-07-05 확정:** 조직 정보 불필요, 가동률 정책 고정값, 단일 테넌트로 확정. 알림 채널만 `SYS_CONFIG`+SCR-017로 신규 정의 완료. 투입률 100% 초과 정책은 경고+허용으로 변경 완료 |
...
│   │   │   └── sys_config.py          # SYS_CONFIG (신규 — 알림 채널 설정)
│   │   ├── schemas/
│   │   │   ├── employee.py
│   │   │   ├── job_type.py
│   │   │   ├── skill.py
...
│   │   │       ├── users.py
│   │   │       ├── audit_logs.py
│   │   │       ├── settings.py         # /api/v1/settings/* (알림 채널, SYS_CONFIG)
│   │   │       └── ai.py
│   │   └── worker.py
│   ├── alembic/
│   ├── tests/
