...
erDiagram
    ...
    HR_JIKMU_MST ||--o{ HR_EMPL_MST : "classified as"
...
> **`JIKMU_ID`** 는 NULL 허용. 기존 데이터 이관 후 점진적으로 등록 가능.
>
...
| `JIKGUP_NM` | VARCHAR(100) | NOT NULL | 직급명 (사원, 대리, 과장, 부장 등) |
...
| `JIKMU_CD` | `JIKMU_NM` | `JIKMU_GRP_CD` | `JIKMU_DESC` |
|---|---|---|---|
...
| `PRJT_ROLE_NM` | VARCHAR(100) | NOT NULL | 프로젝트 내 역할명 (PM, Backend, Frontend, QA 등) |
...
| GET | `/api/v1/employees/{empl_id}` | 사원 상세 조회 |
...
| 기존 Excel 컬럼 | 신규 테이블 | 신규 컬럼 | 비고 |
|---|---|---|---|
| 성명 | `HR_EMPL_MST` | `EMPL_NM` | 사번 없으면 임시 `EMPL_NO` 생성 |
| 팀 | `HR_DEPT_MST` | `DEPT_NM` | 부서 마스터 생성 후 `DEPT_ID` FK 연결 |
| 직급 | `HR_JIKGUP_MST` | `JIKGUP_NM` | 직급 마스터 생성 후 `JIKGUP_ID` FK 연결 |
| 직무 유형 (신규) | `HR_EMPL_MST` | `JIKMU_ID` | 기존 데이터에 없으면 이관 후 수동 보정 |
| 주요기술 | `HR_SKILL_MST` / `HR_EMPL_SKILL_REL` | `SKILL_NM` / `EMPL_ID`+`SKILL_ID` | 쉼표 분리 후 기술 마스터화 |
| 숙련도 | `HR_EMPL_SKILL_REL` | `PRFCY_LEVL` | 전체 기술에 동일 숙련도 적용 또는 수동 보정 |
| 현재 프로젝트명 | `PJT_MST` | `PJT_NM` | 프로젝트 마스터 생성 |
| 역할 | `PJT_ASGN_HIS` | `PRJT_ROLE_NM` | 투입 역할명 |
| 투입률(%) | `PJT_ASGN_HIS` | `ALLOC_RT` | 현재 활성 투입 건으로 생성 |
| 종료 예정일 | `PJT_ASGN_HIS` | `ASGN_END_DT` | 투입 종료 예정일 |
| 가동 가능일 | `HR_AVAIL_SNAP` | `AVAIL_STRT_DT` | DB 직접 저장보다 API 계산 권장 |
...
| 1 | 기존 Excel 데이터 정리 |
| 2 | `EMPL_NO` 사번 부여 |
| 3 | `HR_DEPT_MST`, `HR_JIKGUP_MST`, `HR_JIKMU_MST`, `HR_SKILL_MST`, `PJT_MST` 마스터 생성 |
| 4 | `HR_EMPL_MST` 직원 데이터 Import |
| 5 | `HR_EMPL_SKILL_REL` 기술 스택 Import |
| 6 | `PJT_ASGN_HIS` 현재 투입 정보 Import |
| 7 | `HR_EMPL_MST.JIKMU_ID` 직무 유형 수동 보정 |
| 8 | `HR_AVAIL_SNAP` 가동률 및 추천 결과 검증 |
| 9 | 운영팀 사용자 검수 |
| 10 | 웹 시스템 기준 운영 전환 |
...
7. `HR_JIKMU_MST` 직무 유형 마스터 도입으로 아키텍트·개발자·BA 등 체계적 분류
