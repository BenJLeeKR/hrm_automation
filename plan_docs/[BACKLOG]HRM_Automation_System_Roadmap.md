...
- **리소스 추천 화면 필요 기술 다중 선택 구현 (§9-1 다음 미완료 항목)** — 목록 순서상 다음 미완료 항목인 "필요 기술 선택이 1개만 가능(다중 칩 선택 UI 미구현)"을 처리. `backend/app/repositories/pjt_rcmd_rslt.py`가 `REQ_SKILL_JSON.SKILL_IDS` 배열 전체를 이미 순회해 `skill_match`/`proficiency` 점수를 계산하고 있어 백엔드 변경 없이 프론트엔드만 수정. `recommendations/page.tsx`의 "필요 기술" 필드를 단일 `skill: string` state에서 `skillIds: string[]` 배열로 교체 — Select에서 기술을 고른 뒤 "추가" 버튼을 눌러 배열에 담고, 각 항목을 제거 가능한 칩(`Badge`+`X` 아이콘 버튼)으로 표시, 이미 담긴 기술은 Select 선택지에서 제외해 중복 추가를 UI에서 방지(사원 상세 화면의 "기술 추가" 모달에서 이미 쓰인 것과 동일한 원칙). `handleSubmit`의 `REQ_SKILL_JSON` payload를 `{ SKILL_IDS: skillIds }`로 변경(빈 배열이면 `{}`로 기존과 동일하게 무조건 필터 처리). 백엔드 변경 없음, pytest 107개 그대로 통과. **실 서버 컨테이너에서 실제 HTTP 호출로 검증**: 재빌드 후 실 기술 마스터 2건을 `SKILL_IDS`에 담아 curl로 `POST /api/v1/resource-requests`→`POST /api/v1/recommendations/score` 호출 시 두 기술이 모두 고려된 `skill_match`/`proficiency` 점수가 정상 산출됨을 확인. `/recommendations` 페이지 200 렌더링 확인. §9-1 "리소스 추천 화면 필요 기술 다중 선택" 항목 `[x]` 처리(같은 화면의 "이 후보로 투입 요청" 버튼은 이번 범위에서 다루지 않아 미완료로 유지)
- **프로젝트 상세 화면 "투입 유형 구성" 카드에 계획·종료 인력 수 추가 (사용자 직접 요청)** — 사용자가 "투입 유형 구성에 수행중만 표시하지 말고 계획된 인력과 종료된 인력 수도 표시해달라"고 요청. 기존에는 이 카드가 `ASGN_STAT_CD='ACTIVE'`인 투입만 유형별(수행중/투입준비중/제안중)로 집계해 보여주고 있어, 계획(`PLANNED`)·종료(`DONE`) 상태의 투입은 어디에도 개수가 표시되지 않았음. `projects/[id]/page.tsx`에서 이미 조회해둔 `assignments` 전체 배열을 `ASGN_STAT_CD` 기준으로 추가 집계해(`plannedCount`/`doneCount`) 카드 하단에 "계획"/"종료" 2개 행을 신규 추가(사용자 후속 요청으로 "계획된 인력"/"종료된 인력"에서 간결한 문구로 수정) — 점선 테두리로 기존 유형별 항목과 시각적으로 구분. 신규 API 호출 없이 재사용, 백엔드 변경 없음, pytest 107개 그대로 통과. **실 서버 컨테이너에서 실제 데이터로 검증**: 재빌드 후 실 DB에서 진행/종료가 섞인 프로젝트(ACTIVE 5명·DONE 3명)를 확인, `/projects/{pjt_id}` 페이지 200 렌더링 확인. §9-1 정식 체크리스트 항목이 아닌 사용자 직접 요청 건
- **투입 이력 개별(인력별) 수정 버튼/폼 구현 (§9-1 다음 미완료 항목)** — 목록 순서상 다음 미완료 항목인 "투입 이력 개별(인력별) 수정 버튼/폼 없음"을 처리. 백엔드 `PATCH /api/v1/assignments/{asgn_id}`(투입 유형/역할/투입률/기간/상태/비고 변경 + 100% 초과 재검증)는 이미 구현되어 있어 프론트엔드 연동만 추가. `components/projects/assignment-form-modal.tsx`에 `assignment` prop 전달 시 수정 모드로 동작하도록 확장 — 사원 관리의 `employee-form-modal.tsx`, 프로젝트의 `project-form-modal.tsx`와 동일한 등록/수정 공용 패턴. 대상 사원·프로젝트는 등록 이후 변경할 대상이 아니라(사번/프로젝트 코드와 동일한 원칙) 수정 모드에서는 두 Select를 비활성화하고, 대신 진행 상태(`ASGN_STAT_CD`) Select를 신규로 노출(등록 시에는 항상 서버 기본값 `PLANNED`라 노출하지 않음). 투입 관리 화면(`assignments/page.tsx`)과 프로젝트 상세 화면(`projects/[id]/page.tsx`) 각각의 투입 이력 테이블에 "수정" 아이콘 버튼 열을 추가해 클릭 시 해당 행 데이터로 모달을 채워 여는 방식으로 두 화면 모두 해소. 백엔드 변경 없음, pytest 107개 그대로 통과. **실 서버 컨테이너에서 실제 HTTP 호출로 검증**: 재빌드 후 임시 프로젝트·사원·투입 이력 각 1건을 등록해 curl로 `PATCH /api/v1/assignments/{asgn_id}`(역할/투입률/상태/비고 변경) 호출 시 전부 정상 반영됨을 확인, `/assignments`·`/projects/{pjt_id}` 페이지 200 렌더링 확인, 검증에 사용한 임시 데이터는 SQL로 삭제. §9-1 "투입 이력 개별(인력별) 수정 버튼/폼" 항목 `[x]` 처리 — 투입 관리 섹션 §9-1 항목 완료
- **리소스 추천 화면 "이 후보로 투입 요청" 버튼 구현 (§9-1 다음 미완료 항목)** — 목록 순서상 다음 미완료 항목인 "'이 후보로 투입 요청' 버튼 없음 — 추천 결과 → 실제 투입 등록 연결 흐름 전체 미구현"을 처리. 지난 턴에 등록/수정 공용으로 확장해둔 `components/projects/assignment-form-modal.tsx`를 재사용 — 새 `presetEmplId`/`presetRoleNm`/`presetStartDate`/`presetAllocRt` prop을 추가해, 수정 모드처럼 필드를 잠그지 않고 값만 미리 채워주는 3번째 모드(등록+프리셋)를 지원하도록 확장. `recommendations/page.tsx`의 추천 결과 카드마다 "이 후보로 투입 요청" 버튼을 추가 — 클릭 시 해당 후보(`EMPL_ID`)·직전에 입력한 요청 역할(`role`)·투입 시작 희망일(`availDate`)·최소 투입률(`minAllocRt`)을 프리셋으로 채운 등록 모달이 열리고, 확인 후 `POST /api/v1/assignments`로 그대로 등록되거나 필요 시 값을 조정해 등록 가능. 사원 목록은 `handleSubmit`에서 결과 조인용으로만 쓰고 버리던 `employeesRes.items`를 신규 state로 보관해 재사용(신규 API 호출 없음). 백엔드 변경 없음, pytest 107개 그대로 통과. **실 서버 컨테이너에서 실제 HTTP 호출로 검증**: 임시 프로젝트·사원으로 리소스 요청 등록→추천 실행→"이 후보로 투입 요청" 버튼이 호출하는 것과 동일한 `POST /api/v1/assignments` 호출까지 전체 흐름을 curl로 재현해 정상 등록(201) 확인, `/recommendations` 페이지 200 렌더링 확인, 검증에 사용한 임시 데이터는 SQL로 삭제. §9-1 "리소스 추천 화면 '이 후보로 투입 요청' 버튼" 항목 `[x]` 처리, §9 리스크 "리소스 추천 화면 UI 축소" 해소 처리 — 리소스 추천 섹션 §9-1 항목 전부 완료
...
| 리소스 추천 화면 UI 축소 — 다중 기술 선택·"이 후보로 투입 요청" 버튼 미구현 | 낮음 | 해소 | 설계서(SCR-011)가 요구하는 두 기능 모두 해소됨 — 다중 기술 선택은 2026-07-05 칩 UI로 구현 완료(§9-1 리소스 추천 섹션 참조), "이 후보로 투입 요청" 버튼도 2026-07-05 추천 결과 카드에 추가해 `assignment-form-modal.tsx`(등록/수정 공용 컴포넌트)를 후보 정보로 미리 채워 여는 방식으로 구현 완료 — 실제 권한 검증은 기존 `assignments.create`(A/H/P 허용) 그대로 재사용 | 2026-07-05 |
...
**투입 관리 (`/assignments`)**
- [x] 투입 이력 개별(인력별) 수정 버튼/폼 구현 완료 (2026-07-05). `components/projects/assignment-form-modal.tsx`에 `assignment` prop 전달 시 수정 모드(`PATCH /api/v1/assignments/{asgn_id}`)를 추가 — 사원 관리의 `employee-form-modal.tsx`와 동일한 등록/수정 공용 패턴. 대상 사원·프로젝트 Select는 수정 모드에서 비활성화(등록 후 변경 대상 아님), 대신 진행 상태(`ASGN_STAT_CD`) Select를 신규 노출. 투입 관리 화면(`assignments/page.tsx`)·프로젝트 상세 화면(`projects/[id]/page.tsx`) 각 투입 이력 행에 "수정" 아이콘 버튼 추가
...
- [x] "이 후보로 투입 요청" 버튼 구현 완료 (2026-07-05). 추천 결과 카드마다 버튼을 추가해 클릭 시 `components/projects/assignment-form-modal.tsx`(등록/수정 공용 컴포넌트)를 등록 모드로 열되, 새 `preset*` prop(`presetEmplId`/`presetRoleNm`/`presetStartDate`/`presetAllocRt`)으로 후보 사원·요청 역할·희망 투입일·최소 투입률을 미리 채워 사용자가 확인만 하고 `POST /api/v1/assignments`로 즉시 등록 가능(수정 모드처럼 잠그지 않아 필요 시 값 조정도 가능). 백엔드 변경 없음(기존 등록 API 재사용)
...
**전역 UI (Top Nav)**
- [ ] "내 프로필" 조회·수정 화면 없음 — `top-nav.tsx`의 "내 프로필" 메뉴가 실제 프로필 화면이 아니라 관리자용 사용자 목록(`/settings/users`)으로 이동하며, 로그인 사용자 본인 이름·이메일도 "관리자"/`admin@blueward.co.kr`로 하드코딩되어 있어 실제 로그인 계정 정보가 표시되지 않음(`GET /api/v1/auth/me` 응답을 사용하지 않음). 본인 정보 수정 API 자체도 없음(§9-1 "설정" 섹션의 "계정 수정 기능 없음"과는 별개 — 그 항목은 관리자가 다른 사용자를 수정하는 기능, 이 항목은 로그인 사용자 본인 정보 조회·수정)
- [ ] "설정" 메뉴를 "비밀번호 변경"으로 교체 필요 — 현재 top-nav 드롭다운의 "설정" 메뉴는 관리자 전용 `/settings`(사용자 관리·감사 로그)로 이동하는데, 이는 이미 사이드바 메뉴로도 접근 가능해 top-nav에 중복 노출되어 있음. 비밀번호 변경 API(`PATCH /api/v1/auth/password` 등) 자체가 백엔드에도 없어 신규 구현 필요
- [ ] 상단 "알림"(Bell) 아이콘 표시 내용 없음 — `top-nav.tsx`의 Bell 아이콘은 항상 켜진 붉은 점 배지만 있고 클릭 핸들러·드롭다운·실제 데이터가 전혀 없는 순수 장식 요소. 대시보드가 이미 계산하는 데이터 품질 이슈·투입 종료 예정·투입률 초과 등(`GET /api/v1/dashboard/*`)을 이 알림에 연결할 수 있으나 집계·읽음 처리 방식은 미정
...
| 2026-07-05 | v0.8.18 | 사용자 제안 3건 검토 후 §9-1에 신규 "전역 UI (Top Nav)" 섹션·항목 3건 추가(문서 정리만, 코드 변경 없음) — `top-nav.tsx` 확인 결과 (1) "내 프로필" 메뉴가 실제 프로필 화면이 아니라 관리자용 `/settings/users` 목록으로 이동하고 사용자 이름·이메일도 "관리자"/`admin@blueward.co.kr`로 하드코딩(`GET /api/v1/auth/me` 미사용), (2) "설정" 메뉴가 사이드바와 중복 노출되며 비밀번호 변경 API 자체가 백엔드에 없음, (3) 알림(Bell) 아이콘이 항상 켜진 배지만 있고 클릭 핸들러·실제 데이터 없는 순수 장식 요소임을 확인 — 3건 모두 실제 미구현 공백으로 판단해 §9-1에 등록 | — |
| 2026-07-05 | v0.8.19 | 사용자 요청으로 프로젝트 상세 화면 "투입 유형 구성" 카드 개선 — 기존에는 진행 중(`ASGN_STAT_CD='ACTIVE'`) 투입만 유형별(수행중/투입준비중/제안중)로 표시하고 있어, 계획(`PLANNED`)·종료(`DONE`) 상태의 인력 수도 함께 볼 수 있도록 "계획된 인력"/"종료된 인력" 2개 행을 추가(`projects/[id]/page.tsx`). 신규 API 호출 없이 이미 조회해둔 `assignments` 배열을 상태별로 집계해 재사용. 백엔드 변경 없음, pytest 107개 그대로 통과, 실 서버에서 진행/종료가 섞인 실제 프로젝트(ACTIVE 5명·DONE 3명) 확인 및 `/projects/{pjt_id}` 페이지 200 렌더링 확인. §9-1 체크리스트 항목이 아닌 사용자 직접 요청 건 | — |
| 2026-07-05 | v0.8.20 | 사용자 후속 요청으로 직전 항목의 문구를 "계획된 인력"/"종료된 인력"에서 "계획"/"종료"로 간결하게 수정(`projects/[id]/page.tsx`). 텍스트만 변경, 백엔드 변경 없음, pytest 107개 그대로 통과, `/projects/{pjt_id}` 페이지 200 렌더링 확인 | — |
| 2026-07-05 | v0.8.21 | §9-1 다음 미완료 항목 "투입 이력 개별(인력별) 수정 버튼/폼 없음" 처리 — `components/projects/assignment-form-modal.tsx`에 `assignment` prop 기반 수정 모드(`PATCH /api/v1/assignments/{asgn_id}`) 추가(대상 사원·프로젝트 Select 비활성화, 진행 상태 Select 신규 노출), 투입 관리 화면(`assignments/page.tsx`)·프로젝트 상세 화면(`projects/[id]/page.tsx`) 각 투입 이력 행에 "수정" 아이콘 버튼 추가. 백엔드 변경 없음, pytest 107개 그대로 통과, 실 서버 curl로 `PATCH`(역할/투입률/상태/비고 변경) 정상 반영 확인(검증용 임시 데이터는 삭제). §9-1 "투입 이력 개별(인력별) 수정 버튼/폼" 항목 `[x]` 처리 — 투입 관리 섹션 §9-1 항목 완료 | — |
| 2026-07-05 | v0.8.22 | §9-1 다음 미완료 항목 "리소스 추천 '이 후보로 투입 요청' 버튼 없음" 처리 — `assignment-form-modal.tsx`에 `presetEmplId`/`presetRoleNm`/`presetStartDate`/`presetAllocRt` prop 신규 추가(등록 모드에서 필드를 잠그지 않고 값만 미리 채움), `recommendations/page.tsx` 추천 결과 카드마다 "이 후보로 투입 요청" 버튼을 추가해 클릭 시 후보·요청 조건이 채워진 등록 모달을 엶. `handleSubmit`에서 조인용으로만 쓰던 사원 목록을 state로 보관해 재사용. 백엔드 변경 없음, pytest 107개 그대로 통과, 실 서버 curl로 요청 등록→추천 실행→투입 등록 전체 흐름 재현 확인(검증용 임시 데이터는 삭제). §9-1 "리소스 추천 '이 후보로 투입 요청' 버튼" 항목 `[x]` 처리, §9 리스크 "리소스 추천 화면 UI 축소" 해소 — 리소스 추천 섹션 §9-1 항목 전부 완료 | — |
